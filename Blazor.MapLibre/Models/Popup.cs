...
namespace Blazor.MapLibre.Models
{
    public class Popup : IDisposable
    {
        ...
        /// </summary>
        public readonly string ContainerId = "maplibrepopup_" + Guid.NewGuid();
        ...
        public async Task<Listener> AddListener(Events eventName, Action handler)
        {
            ...
            await _Module.InvokeVoidAsync("MapLibrePopup.on", ContainerId, eventName.ToString(), reference);
        ...
        public async ValueTask AddClassName(string className) => await _Module.InvokeVoidAsync($"MapLibrePopup.addClassName", ContainerId, className);

        /// <summary>
        /// Adds the popup to a map.
        /// </summary>
        /// <param name="mapLibreMap">The MapLibre GL JS map to add the popup to.</param>
        public async ValueTask AddTo(MapLibreMap mapLibreMap) => await _Module.InvokeVoidAsync($"MapLibrePopup.addTo", ContainerId, _MapId);
        ...
        public async ValueTask<LngLat> GetLngLat() => await _Module.InvokeAsync<LngLat>($"MapLibrePopup.getLngLat", ContainerId);

        /// <summary>
        /// Returns the popup's maximum width.
        /// </summary>
        /// <returns>The maximum width of the popup.</returns>
        public async ValueTask<string> GetMaxWidth() => await _Module.InvokeAsync<string>($"MapLibrePopup.getMaxWidth", ContainerId);

        /// <summary>
        /// </summary>
        /// <returns>true if the popup is open, false if it is closed.</returns>
        public async ValueTask<bool> IsOpen() => await _Module.InvokeAsync<bool>($"MapLibrePopup.isOpen", ContainerId);

        /// <summary>
        /// Removes the popup from the map it has been added to.
        /// </summary>
        public async ValueTask Remove() => await _Module.InvokeVoidAsync($"MapLibrePopup.remove", ContainerId);

        /// <summary>
        /// Removes a CSS class from the popup container element.
        /// </summary>
        /// <param name="className">Non-empty string with CSS class name to remove from popup container.</param>
        public async ValueTask RemoveClassName(string className) => await _Module.InvokeVoidAsync($"MapLibrePopup.removeClassName", ContainerId, className);
        ...
        public async ValueTask SetHtml(string html) => await _Module.InvokeVoidAsync($"MapLibrePopup.setHTML", ContainerId, html);

        /// <summary>
        /// Sets the geographical location of the popup's anchor, and moves the popup to it. Replaces trackPointer() behavior.
        /// </summary>
        /// <param name="lngLat">The geographical location to set as the popup's anchor.</param>
        public async ValueTask SetLngLat(LngLat lngLat) => await _Module.InvokeVoidAsync($"MapLibrePopup.setLngLat", ContainerId, lngLat);
        ...
        public async ValueTask SetText(string text) => await _Module.InvokeVoidAsync($"MapLibrePopup.setText", ContainerId, text);
        ...
        public async ValueTask<bool> ToggleClassName(string className) => await _Module.InvokeAsync<bool>($"MapLibrePopup.toggleClassName", ContainerId, className);
